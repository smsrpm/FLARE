...
def load_containment_results(case_name, run_dir=None):
    """Load integrated FLARECON CSV (<case>-CON.csv), if present."""
    for search_dir in ([run_dir] if run_dir is not None else []) + [WORK_DIR]:
        if search_dir is None:
            continue
        p = Path(search_dir) / f"{case_name}-CON.csv"
        if p.exists():
            try:
                return pd.read_csv(p)
            except Exception:
...
def containment_scalar_summary(con_df, use_english=False):
    ...
    if status:
        cls = ("danger" if "Detonation" in status
               else ("warn" if "Deflagration" in status else "ok"))
        items.append((status, "Hydrogen flammability", cls, True))
    return items


def containment_flammability_status(con_df):
    if con_df is None or con_df.empty:
        return None
    if "H2 Concentration [vol%]" not in con_df.columns:
        return None
    h2 = pd.to_numeric(con_df["H2 Concentration [vol%]"], errors="coerce").fillna(0.0) / 100.0
    steam_col = "Steam Concentration [vol%]"
    steam = (pd.to_numeric(con_df[steam_col], errors="coerce").fillna(0.0) / 100.0) if steam_col in con_df.columns else pd.Series(np.zeros(len(con_df)))
    deflag = (steam < 0.55) & (h2 >= 0.04) & (h2 <= 0.75)
    deton = deflag & (h2 >= 0.18) & (h2 <= 0.59) & (steam < 0.35)
    if bool(deton.any()):
        return "Detonation-region entry"
    if bool(deflag.any()):
        return "Deflagration-region entry"
...
def _sim_all_load_status(run_dir=None, *, reattach_active=True):
    if run_dir is None:
        run_dir = st.session_state.get("sim_all_run_dir")
    if not run_dir and reattach_active:
        # After a browser refresh, only reattach to a still-active batch.
        # Do not resurrect the last completed Run All Cases status.
        run_dir = _sim_all_latest_status_dir(active_only=True)
        if run_dir:
            st.session_state.sim_all_run_dir = run_dir
    if not run_dir:
        return {}
...
with tab_run:
    ...
    if run_btn and status != "running":
        # A single-case run should not continue to display a stale Run All Cases
        # completion/status banner from an earlier batch.
        st.session_state.pop("sim_all_run_dir", None)
        st.session_state.console_log = ""
        ...
        # When overrides are active the temp workbook is named .~<case>_run_in.xlsx.
        ...
        if overrides:
            ...
            _run_input_src = selected_input_path
        ...
        for _line in _proc.stdout:
            ...
            if _line.startswith("SIMTIME "):
                ...
                try:
                    ...
                    _status_slot.info(f"🔄  FLARE RUNNING  t = {_t:.0f} s")
                except ValueError:
                    pass
            elif _stripped == "POST-PROCESSING":
                _status_slot.info("📊  FLARE POST-PROCESSING…")
            elif _stripped == "FLARECON_PRE_PROCESSING":
                _status_slot.info("🏢  FLARECON PRE-PROCESSING…")
            elif _stripped == "FLARECON_SIMULATION":
                _status_slot.info("🏢  FLARECON RUNNING…")
            elif _line.startswith("FLARECON_SIMTIME "):
                try:
                    _t = float(_line.split()[1])
                    _status_slot.info(f"🏢  FLARECON RUNNING  t = {_t:.0f} s")
                except ValueError:
                    pass
            elif _stripped == "FLARECON_POST_PROCESSING":
                _status_slot.info("🏢  FLARECON POST-PROCESSING…")
        _proc.wait()
